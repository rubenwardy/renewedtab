...
async function tryGetBookmark(id: string): Promise<browser.bookmarks.BookmarkTreeNode | null> {
	if (app_version.is_debug && (browser as any).is_ui_test == undefined && id == "1") {
		return createDebugData();
	}

	try {
		const toolbar_subtree = await browser.bookmarks.get(id);
...
export async function getBookmarksBarId(): Promise<string | undefined> {
	return (await tryGetBookmark("1") ?? await tryGetBookmark("toolbar_____"))?.id;
}
...
async function getBookmarks(includeFoldersAsSections: boolean, onSelectFolder?: (link: Link) => void): Promise<Link[]> {
	const bookmarks = await tryGetSubTree("1") ?? await tryGetSubTree("toolbar_____");
	if (!bookmarks) {
		throw new UserError(miscMessages.errFetchBookmarks);
	}

	let ret = convertToLinks(bookmarks).children!;
	if (includeFoldersAsSections) {
		ret = ret.flatMap(link => {
			if (!link.children) {
				return [ link ];
			}
...
export default function useBookmarks(includeFoldersAsSections: boolean, onSelectFolder?: (link: Link) => void) {
	return usePromise(() => getBookmarks(includeFoldersAsSections, onSelectFolder), [includeFoldersAsSections]);
}
